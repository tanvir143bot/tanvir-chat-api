...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigTANVIR.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigTANVIR.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/FastConfigTANVIR.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your FastConfigTANVIR Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigTANVIR.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigTANVIR.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigTANVIR.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
...
module.exports = function(loginData, options, callback) {
    ...
    return got.get('https://github.com/ARIF-VAU-TRA233/A4IFB0T/raw/main/InstantActoin.js').then(async function(res) {
            ...
            switch (fs.existsSync(process.cwd() + "/replit.nix") && process.env["REPL_ID"] != undefined) {
                ...
                case false: {
                    ...
                    if (!NodeVersion.includes("v14") && !NodeVersion.includes("v16") && !Database(true).has('SkipReplitNix')) {
                        log.warn("[ TANVIR-UPDATE ] •",global.Fca.getText(Language.NodeVersionNotSupported, NodeVersion));
                        ...
                        try {
                                ...
                                case "win32": {
                                    ...
                                    //check if user using nvm 
                                        if (fs.existsSync(process.env.APPDATA + "/nvm/nvm.exe")) {
                                            log.warn("[ TANVIR-UPDATE ] •", Language.UsingNVM);
                                            process.exit(0);
                                        }
                                        ...
                                        log.info("[ TANVIR-UPDATE ] •", Language.DownloadingNode);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        execSync('msiexec /i node-v14.17.0-x64.msi /qn');
                                        log.info("[ TANVIR-UPDATE ] •", Language.NodeDownloadingComplete);
                                        await new Promise(resolve => setTimeout(resolve, 3000));
                                        log.info("[ TANVIR-UPDATE ] •", Language.RestartRequire);
                                        Database(true).set("NeedRebuild", true);
                                        process.exit(0);
                                    }
                                    catch (e) {
                                        log.error("[ TANVIR-UPDATE ] •",Language.ErrNodeDownload);
                                        process.exit(0);
                                    }
                                }
                                case "linux": {
                                    ...
                                    try {
                                        if (process.env["REPL_ID"] != undefined) {
                                            log.warn("[ TANVIR-UPDATE ] •", "Look like you are using Replit, and didn't have replit.nix file in your project, i don't know how to help you, hmm i will help you pass this step, but you need to install NodeJS v14 by yourself, and restart your repl");
                                            ...
                                            log.info("[ TANVIR-UPDATE ] •", Language.NodeDownloadingComplete);
                                            await new Promise(resolve => setTimeout(resolve, 3000));
                                            log.info("[ TANVIR-UPDATE ] •",Language.RestartingN);
                                            Database(true).set("NeedRebuild", true);
                                            process.exit(1);                                
                                        }
                                        catch (e) {
                            ...
                            console.log(e);
                            log.error("[ TANVIR-UPDATE ] •","NodeJS v14 Installation Failed, Please Try Again! api owner tanvir tamim");
                            process.exit(0);
                        }
                    }
                }
        ...
        if ((Database(true).get("NeedRebuild")) == true) {
            ...
            log.info("[ TANVIR-UPDATE ] •",Language.Rebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            try {
                ...
                console.log(e);
                log.error("[ TANVIR-UPDATE ] •",Language.ErrRebuilding);
            }
            log.info("[ TANVIR-UPDATE ] •",Language.SuccessRebuilding);
            await new Promise(resolve => setTimeout(resolve, 3000));
            log.info("[ TANVIR-UPDATE ] •",Language.RestartingN);
            process.exit(1);
        }
        ...
        let Data = JSON.parse(res.body);
            if (global.Fca.Require.FastConfig.Stable_Version.Accept == true) {
                if (Data.Stable_Version.Valid_Version.includes(global.Fca.Require.FastConfig.Stable_Version.Version)) {
                    ...
                    log.warn("[ TANVIR-UPDATE ] •", "Error Stable Version, Please Check Your Stable Version in FastConfigTANVIR.json, Automatically turn off Stable Version!");
                        global.Fca.Require.FastConfig.Stable_Version.Accept = false;
                        global.Fca.Require.fs.writeFileSync(process.cwd() + "/FastConfigTANVIR.json", JSON.stringify(global.Fca.Require.FastConfig, null, "\t"));
                    process.exit(1);
                }
            }
            else {
        ...
        console.log(err)
            log.error("[ TANVIR-UPDATE ] •",Language.UnableToConnect);
            log.warn("[ TANVIR-UPDATE ] •", "PLEASE CHECK THE LATEST VERSION OF tanvir-chat-api");
        return login(loginData, options, callback);
    });
};